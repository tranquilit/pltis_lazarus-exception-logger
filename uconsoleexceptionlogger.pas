...
begin
  ...
  //(AOwner as TCustomApplication).OnException := @HandleException;
  //CustomApplication.Flags := Application.Flags - [AppNoExceptionMessages];
...
begin
  ...
  {$ENDIF}
  // App Info
  SubAddLine(SExeName, ExtractFileName(ParamStr(0)));
  //SubAddLine(SExeName, ExtractFileName(Application.ExeName));
  SubAddLine(SApplicationTitle, GetAppTitle );
...
begin
  ...
  Writeln(FLastException.Message);

  try
    biFormatted := FormatBasicDataReport(FBasicData);
    WriteLn(biFormatted.Text);
  finally
    biFormatted.free;
  end;
